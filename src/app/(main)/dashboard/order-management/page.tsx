...
export default function OrdersListPage() {
  ...
  const pagination = data?.data.pagination ?? { total: 0, totalPages: 1, currentPage: 1, limit: 9 };
  ...
  return (
    <div className="flex flex-col gap-6 p-2">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="dark:text-primary text-2xl font-semibold text-gray-900">Order&#39;s List</h1>
          <p className="mt-1 text-sm text-gray-500">Manage all orders available across dark stores and regions</p>
        </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-baseline gap-2 pb-2">
                  <h3 className="text-muted-foreground text-2xl font-bold">{stat.value}</h3>
                  {stat.color === "purple" && <span className="h-2 w-10 bg-purple-600"></span>}
                  {stat.color === "orange" && <span className="h-2 w-10 bg-orange-600"></span>}
                  {stat.color === "green" && <span className="h-2 w-10 bg-green-600"></span>}
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select value={selectedStatus} onValueChange={(v) => setSelectedStatus(v as GetOrdersParams["status"])}>
          <SelectTrigger className="border-muted bg-muted text-muted-foreground h-16 w-[140px]">
          ...
          {showDateFilter && (
            <div className="absolute top-12 z-50 flex w-64 flex-col gap-3 rounded-md border bg-white p-4 shadow-md">
              ...
              <div className="flex gap-2">
                <Button className="flex-1" size="sm" onClick={() => setShowDateFilter(false)}>
                  Apply
                </Button>
                <Button
                  variant="outline"
        ...
        <div className="relative">
          ...
          {showFilters && (
            <div className="absolute top-12 z-50 flex w-64 flex-col gap-3 rounded-md border bg-white p-4 shadow-md">
              ...
              <div className="flex gap-2">
                <Button className="mt-2 flex-1" onClick={() => setShowFilters(false)}>
                  Apply
                </Button>
                <Button
                  variant="outline"
      ...
      <OrderDetailsDialog order={selectedOrder} isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} />
    </div>
  );
}
