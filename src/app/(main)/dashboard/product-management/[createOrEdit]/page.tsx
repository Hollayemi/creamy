...
import { useGetCategoriesQuery, type Category } from "@/stores/services/settingsApi";
...
const regionStockSchema = z.object({
  region: z.string(),
  mainProduct: z.number().min(1, "Main product stock is required"),
  variants: z.array(regionVariants).optional(),
});
...
export default function AddNewProductPage({ searchParams }: any) {
  ...
  useEffect(() => {
    if (!fetchError && productData?.data) {
      ...
      if (fetched.variants && fetched.variants.length > 0) {
        setVariants(
          fetched.variants.map((v: any, index: number) => ({
            id: String(index + 1),
            sku: v.sku || "",
  ...
  const handleRemoveVariant = (variantId: string) => {
    ...
    // Remove the corresponding variant entry from every region row
    if (removedIndex !== -1) {
      setRegions((prev) =>
        prev.map((r) => {
          const updatedVariants = [...r.variants];
          updatedVariants.splice(removedIndex, 1);
    ...
    try {
      const payload = {
        ...
        regionDistribution: regions.map((r) => ({
          region: r.region,
          mainProduct: Number(r.mainProduct) || 0,
      ...
      <div className="flex-1">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-2 lg:grid-cols-5">
            <div className="space-y-6 rounded-xl border lg:col-span-3">
              <h1 className="m-5 text-xl font-[500] capitalize">
                {showing === "create" ? "Add New" : showing} Product
              </h1>
                ...
                <CardContent className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input
                        className="bg-muted mb-4 h-10 w-full! min-w-full"
                        id="productName"
                        value={productName}
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Select value={category} onValueChange={setCategory}>
                        <SelectTrigger className="bg-muted h-11! w-full" id="category">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {categories.map((e, i) => (
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={status} onValueChange={(v) => setStatus(v as "active" | "inactive" | "draft")}>
                        <SelectTrigger className="bg-muted h-11! w-full" id="status">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={unitType} onValueChange={setUnitType}>
                        <SelectTrigger className="bg-muted h-11! w-full" id="unitType">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {unitTypes.map((e, i) => (
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                      ...
                      <MultiSelect
                        ...
                        customOnchange={(value: string) =>
                          setRegions((prev) => {
                            ...
                            return [...prev, { region: value, mainProduct: "0", variants: [] }];
                          })
                        }
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={stockAlert} onValueChange={setStockAlert}>
                        <SelectTrigger className="bg-muted h-11! w-full" id="stockAlert">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
            ...
            <div className="space-y-6 lg:col-span-2">
              <Card className="border-0 shadow-none">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="bg-muted flex min-h-[50px] flex-wrap gap-2 rounded-md border p-2">
                    {tags.map((tag) => (
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    {existingImages.map((img, index) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="absolute top-1 right-1 rounded-full bg-white/90 p-1 opacity-0 shadow-md transition-opacity group-hover:opacity-100 hover:bg-white"
                        >
                    ...
                    {existingImages.length + images.length < 6 && (
                      <label className="border-input hover:bg-accent/50 flex aspect-square cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed transition-colors">
                        ...
                        <span className="text-muted-foreground px-2 text-center text-xs">Click to upload</span>
                  ...
                  <p className="text-muted-foreground text-xs">
                    ...
                    {existingImages.length > 0 &&
                      ` (${existingImages.length} existing + ${images.length} new = ${existingImages.length + images.length} total)`}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="border-none shadow-none">
                ...
                <CardContent>
                  {variants.length === 0 ? (
                    <p className="text-muted-foreground py-4 text-center text-sm">No variants added yet</p>
                  ) : (
                    variants.map((variant, index) => (
                      <div key={variant.id} className="mb-6 border-b pb-4 last:mb-0 last:border-b-0">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                          ...
                          <div className="space-y-2">
                            ...
                            >
                              <SelectTrigger className="bg-muted h-11 w-full">
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                {unitTypes.map((e, i) => (
                ...
                <CardContent>
                  <div className="space-y-3">
                    {/* Header */}
                    <div
                      className="grid gap-4 border-b pb-2 text-sm font-medium"
                      style={{ gridTemplateColumns: `repeat(${variants.length + 2}, minmax(0, 1fr))` }}
                    >
                      <div>Region</div>
                      ...
                      regions.map((region, index) => (
                        ...
                        >
                          <div className="text-sm font-medium">
                            {availableRegions.find((r) => r.id === region.region)?.displayName || region.region}
                          </div>
                          ...
                          {variants.map((v, vIndex) => (
                            <Input
                              ...
                              className="bg-muted h-9 w-full"
                              placeholder="0"
                              value={region.variants?.[vIndex]?.quantity || ""}
                              onChange={(e) =>
                                setRegions((prev) =>
                                  prev.map((r) => {
                                    if (r.region !== region.region) return r;

                                    const updatedVariants = [...(r.variants || [])];
                                    updatedVariants[vIndex] = { variantId: v.id, quantity: e.target.value };
