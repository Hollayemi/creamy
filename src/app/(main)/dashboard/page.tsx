...
export default function Page() {
  ...
  const regionalOrders = [
    { region: "Surulere", amount: 120000 },
    { region: "Agege", amount: 80000 },
    { region: "Ikeja", amount: 50000 },
    { region: "Lekki", amount: 40000 },
  ];
  ...
  const calculateMetrics = (current: number[], previous: number[]) => {
    ...
    const percent = previousTotal ? ((diff / previousTotal) * 100).toFixed(1) : "0";
  ...
  const salesMetrics = calculateMetrics(
    currentMonthOrders.map((o) => o.amount),
    previousMonthOrders.map((o) => o.amount),
  );

  const ordersMetrics = calculateMetrics([currentMonthOrders.length], [previousMonthOrders.length]);

  const getUniqueCustomers = (orders: typeof currentMonthOrders) => {
    return new Set(orders.map((o) => o.customer)).size;
  };

  const currentCustomers = getUniqueCustomers(currentMonthOrders);
  const previousCustomers = getUniqueCustomers(previousMonthOrders);

  const customersMetrics = calculateMetrics([currentCustomers], [previousCustomers]);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-[327px_1fr] gap-6">
        ...
        <div className="flex flex-col gap-6">
          {/* Stats Cards Row */}
          <div className="grid h-[122px] grid-cols-3 gap-6">
            <StatsCard
              title="Total Sales"
              value={`₦${salesMetrics.total.toLocaleString()}`}
              change={`${salesMetrics.percent}%`}
              // positive={salesMetrics.positive}
              data={currentMonthOrders.map((o) => ({ value: o.amount }))}
              icon={<DollarSign />}
            />

            <StatsCard
              title="Completed Orders"
              value={ordersMetrics.total.toLocaleString()}
              change={`${ordersMetrics.percent}%`}
              // positive={ordersMetrics.positive}
              data={currentMonthOrders.map((_, i) => ({ value: i + 1 }))}
              icon={<ShoppingCart />}
            />

            <StatsCard
              title="Active Customers"
              value={customersMetrics.total.toLocaleString()}
              change={`${customersMetrics.percent}%`}
              // positive={customersMetrics.positive}
              data={currentMonthOrders.map((_, i) => ({ value: i + 1 }))}
              icon={<Users />}
            />
          </div>
          ...
          <div className="flex-1">
            <OverviewChart change={0} positive={false} />
          </div>
        </div>
      </div>

      {/* RECENT ORDERS + REGIONAL PERFORMANCE */}
      {/* <div className="grid h-[397px] grid-cols-[3fr_1fr] gap-6">
        <RecentOrders orders={orders} setOrders={setOrders} />
        <RegionalPerformance orders={regionalOrders} />
      </div> */}
