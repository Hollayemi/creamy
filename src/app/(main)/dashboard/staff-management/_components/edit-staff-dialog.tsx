...
import { useUpdateStaffRoleMutation, useGetRolesQuery, type Staff } from "@/stores/services/staffApi";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Update {staff.fullName}&apos;srole and permissions</DialogDescription>
        </DialogHeader>

        <div className="space-y-6 py-4">
          {/* Current User Info */}
          <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
            <p className="text-muted-foreground text-sm">Editing permissions for</p>
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="manualPermissions" className="cursor-pointer font-normal">
              ...
              <div className="max-h-[400px] space-y-2 overflow-y-auto rounded-lg border p-4">
                {permissions.map((permission) => {
                  ...
                  return (
                    <div
                      key={permission}
                      className="flex items-start space-x-3 rounded-lg p-3 hover:bg-gray-50 dark:hover:bg-gray-700"
                    >
                      <Checkbox
                        id={`edit-${permission}`}
                        checked={isSelected}
                      ...
                      <div className="flex-1">
                        <Label htmlFor={`edit-${permission}`} className="cursor-pointer font-medium">
                          {permission}
                          {isRolePermission && (
                            <span className="ml-2 rounded bg-orange-50 px-2 py-0.5 text-xs text-orange-600">
                              From Role
                            </span>
                          )}
                        </Label>
                        <p className="text-muted-foreground text-sm">{permission.split("_").join(" ")}</p>
                      </div>
                    </div>
                  );
                })}
